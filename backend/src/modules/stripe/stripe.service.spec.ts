...
describe('StripeService payment webhook handling', () => {
  ...
  it('compensates inventory on checkout.session.expired for pending orders', async () => {
    ...
    (prisma.$transaction as jest.Mock).mockImplementation(async (fn: (client: typeof tx) => Promise<unknown>) =>
      fn(tx),
    );
    ...
    expect(inventory.compensateStripeCheckout).toHaveBeenCalledWith(expect.anything(), 'order-abc', 'user-1');
  });
