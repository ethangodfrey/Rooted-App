 ...
 * pos-inventory-ingest BullMQ queue. Workers apply dual-channel stock updates
 * so POS dashboards and activity feeds refresh without manual sync.
 */
export async function enqueueOnlineSaleDeductions(
  ...
  const { resolveRedisConnection } = await import('./redis-connection');
