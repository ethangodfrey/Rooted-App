...
**Current architecture:** `mobile/` (Expo) + `web/` (Vite/React) + `backend/` (NestJS API for POS, markets agent, admin agents) + Supabase (Auth, Postgres, Storage). See [`docs/LAUNCH_RUNBOOK.md`](./LAUNCH_RUNBOOK.md) for production setup.

**Stack:** Expo SDK 54 · Expo Router · TypeScript · Supabase · NativeWind · NestJS (POS & market pipelines)

**Source of truth:** [`Context - Rooted - Marketplace App.txt`](./Context%20-%20Rooted%20-%20Marketplace%20App.txt) · Product spec · Monorepo profile/structure
...
4. **Direct Supabase + backend API.** Mobile/web use Supabase client + RLS for core marketplace data; NestJS handles POS OAuth, webhooks, and market/admin agents.
