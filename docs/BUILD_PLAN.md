...
**Approach:** Monorepo with `mobile/` (Expo), `web/` (Vite), and `backend/` (NestJS + Prisma). Mobile and web talk to Supabase directly for most features; backend handles POS, market agents, and admin tooling.

**Stack:** Expo SDK 54 · Expo Router · Vite + React · NestJS · Prisma · TypeScript · Supabase (Auth + Postgres + Storage) · NativeWind (mobile)

**Source of truth:** [`Context - Rooted - Marketplace App.txt`](./Context%20-%20Rooted%20-%20Marketplace%20App.txt) · Product spec · [`LAUNCH_RUNBOOK.md`](./LAUNCH_RUNBOOK.md) for production setup
...
4. **Supabase-first.** Mobile and web use Supabase client + RLS for auth, data, and storage. Backend API is used for POS sync, market pipelines, and admin agents.
