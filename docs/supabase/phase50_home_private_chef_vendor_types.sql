-- Vendorly Phase 50 — Home Chef / Private Chef / Micro-Brand personas + fulfillment
-- Apply after phase49.
...
comment on column public.vendors.shipping_enabled is
  'Phase 50: Micro-Brand enables nationwide shipping at checkout.';
comment on column public.vendors.flat_rate_shipping_fee_cents is
  'Phase 50: Flat-rate shipping fee in cents (waived above free_shipping_minimum_cents).';
comment on column public.vendors.free_shipping_minimum_cents is
  'Phase 50: Order subtotal (cents) that unlocks free shipping.';
comment on column public.products.has_variants is
  'Phase 50: product uses variants JSON for size/color/stock combos.';
comment on column public.products.variants is
  'Phase 50: { attributes: [{name, values}], combinations: [{id, options, price_cents, stock}] }.';
