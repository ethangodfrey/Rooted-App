...
2. `PosSalesIngestService` calls `upsertTransaction()` **only after a successful ledger write** (`result?.id`). Failures are non-fatal.
3. `pos-sales-ingest.processor.ts` logs `processor: phase47=...` on success.
