...
export function providerNotEnabledMessage(provider: OAuthProvider): string {
  const providerLabel = provider === 'google' ? 'Google' : 'Apple';
  return `${providerLabel} sign-in isn't enabled yet. Enable it in Supabase Dashboard → Authentication → Providers → ${providerLabel}.`;
}
...
function isProviderNotEnabled(text: string, body?: SupabaseAuthErrorBody | null): boolean {
  const haystack = [text, body?.msg, body?.error, body?.error_description]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
...
export function formatOAuthError(error: unknown, provider?: OAuthProvider): string {
  ...
  if (typeof error === 'string') {
    ...
    if (isProviderNotEnabled(error, body) && provider) return providerNotEnabledMessage(provider);
    return messageFromBody(body, error);
  }

  if (error instanceof Error) {
    const body = parseSupabaseErrorPayload(error.message);
    if (isProviderNotEnabled(error.message, body) && provider) {
      return providerNotEnabledMessage(provider);
    }
  ...
  if (typeof error === 'object') {
    ...
    const text = [obj.msg, obj.error, obj.error_description].filter(Boolean).join(' ');
    if (isProviderNotEnabled(text, obj) && provider) return providerNotEnabledMessage(provider);
...
export function formatOAuthUrlError(
  ...
  if (isProviderNotEnabled(combined, body) && provider) return providerNotEnabledMessage(provider);
