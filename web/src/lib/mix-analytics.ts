/** Vendor matchmaking & market mix analytics helpers. */
...
/** Orange / amber / gold shades for the donut chart. */
...
function nextWeekdayLabel(from: Date = new Date()): string {
  ...
  d.setDate(d.getDate() + ((2 - d.getDay() + 7) % 7 || 7)); // next Tuesday-ish label helper
  return d.toLocaleDateString('en-US', { weekday: 'long' });
}

/**
 * Density heuristics: warn when one bucket dominates or under-served categories exist.
 */
...
export function pickInviteCandidates(
  ...
  // Fill remaining slots with any off-duty approved vendors if preferred buckets are scarce.
...
export function neededBucketsFromRecommendations(recs: MixRecommendation[]): MixBucket[] {
  const buckets = recs
    .map((r) => r.targetBucket)
    .filter((b): b is MixBucket => Boolean(b));
