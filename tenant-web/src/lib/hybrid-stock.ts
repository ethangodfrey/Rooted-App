...
/** Unit split, e.g. "12 Pre-Order / 5 Walk-Up". */
export function formatHybridStockLabel(split: HybridStockSplit): string {
  return `${split.preOrder} Pre-Order / ${split.walkUp} Walk-Up`;
}

/** Percent split shown on the hybrid allocator, e.g. "70% Pre-Order / 30% Walk-Up". */
export function formatHybridStockPercentLabel(split: HybridStockSplit): string {
  return `${split.preOrderPercent}% Pre-Order / ${100 - split.preOrderPercent}% Walk-Up`;
}
