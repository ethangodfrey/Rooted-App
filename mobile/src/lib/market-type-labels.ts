...
export function isShopperMarketEvent(
  event: Pick<Event, 'market_type' | 'name'> & { sync_metadata?: Event['sync_metadata'] },
): boolean {
...
export function filterShopperEvents<T extends Pick<Event, 'market_type' | 'name'> & { sync_metadata?: Event['sync_metadata'] }>(
  events: T[],
): T[] {
