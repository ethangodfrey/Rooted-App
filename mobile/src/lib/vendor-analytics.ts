...
/** Local calendar date — must match how buildDaySeries labels each day. */
...
/** Zoom charts to days with activity so a lone sale is not off-screen at day 30. */
...
async function fetchVendorAnalyticsUncached(
  ...
  for (const tx of txs) {
    ...
    if (!isSale) continue;
    // sale_pos is counted from the POS API when available to avoid double-counting.
