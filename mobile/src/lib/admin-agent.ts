...
export async function fetchFeedbackStats(): Promise<FeedbackStats | null> {
  ...
  try {
    return await api.get<FeedbackStats>('/admin/vendors/feedback/stats');
  } catch {
    return null;
  }
}
...
export const RECOMMENDATION_COLOR: Record<VendorReviewRecommendation, string> = {
  approve: 'text-forest',
  reject: 'text-danger',
  needs_review: 'text-warn',
};

export const OUTCOME_LABEL: Record<ReviewFeedbackOutcome, string> = {
  accepted: 'Matched AI',
  overridden: 'Overrode AI',
  no_ai_suggestion: 'No AI suggestion',
};
