...
export default function ChefPortfolioScreen() {
  const { chef } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    const data = await fetchExploreContentForCreator({ creatorType: 'chef', chefId: chef.id });
    setItems(data);
    setLoading(false);
  }, [chef?.id]);
  ...
  function confirmDelete(item: ExploreContent) {
    Alert.alert('Delete post', 'Remove this from your portfolio?', [
  ...
  return (
    <Screen scroll>
      ...
      <Text variant="subtitle" className="mb-5">
        Showcase dishes, events, and behind-the-scenes moments in the customer Explore feed.
      </Text>

      <Button
        className="mb-6"
        label="Add to portfolio"
        onPress={() => router.push('/(chef)/explore/new')}
      />
      ...
      {loading ? (
        ...
        <Text variant="caption">
          Your portfolio is empty. Tap &ldquo;Add to portfolio&rdquo; to share your first photos.
        </Text>
      ) : (
